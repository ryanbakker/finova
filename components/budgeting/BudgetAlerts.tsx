...
export function BudgetAlerts({
    ...
    return (
      <Card className="border-l-4 border-l-sky-500 h-full">
        <CardHeader className="pb-2">
          <div className="flex items-center space-x-2 text-sky-700 dark:text-sky-300 text-sm mb-2">
  ...
  return (
    <Card className="border-l-4 border-l-sky-500 h-full pb-4">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center space-x-2 text-sky-700 dark:text-sky-300 text-sm">
      ...
      <CardContent className="pt-0 pb-1">
        <div className="space-y-2 max-h-32 overflow-y-auto">
          {criticalAlerts.length > 0 ? (
            criticalAlerts.map((alert, index) => (
              <div
                ...
                className={`flex items-center space-x-2 p-2 border rounded-lg ${
                  alert.color === "red"
                    ? "bg-gradient-to-r from-sky-50 to-white dark:from-sky-950/30 dark:to-neutral-900/50 border-sky-200 dark:border-sky-800"
                    : "bg-gradient-to-r from-sky-50 to-white dark:from-sky-950/30 dark:to-neutral-900/50 border-sky-200 dark:border-sky-800"
                }`}
              >
                {alert.color === "red" ? (
                  <AlertOctagon className="h-4 w-4 text-sky-600 dark:text-sky-400 flex-shrink-0" />
                ) : (
                  <AlertTriangle className="h-4 w-4 text-sky-600 dark:text-sky-400 flex-shrink-0" />
                )}
                <div className="min-w-0 flex-1">
                  <p
                    className={`font-medium text-xs truncate text-sky-800 dark:text-sky-200`}
                  >
            ...
            <div className="flex items-center space-x-2 p-2 bg-gradient-to-r from-sky-50 to-white dark:from-sky-950/30 dark:to-neutral-900/50 border border-sky-200 dark:border-sky-800 rounded-lg">
          ...
          {categoryData.length > 0 && (
            <div className="flex items-center space-x-2 p-2 bg-gradient-to-r from-sky-50 to-white dark:from-sky-950/30 dark:to-neutral-900/50 border border-sky-200 dark:border-sky-800 rounded-lg">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-sky-700 dark:text-sky-300">
                  {goodCategories.length} on track, {warningCategories.length}{" "}
                  warnings, {overBudgetCategories.length} over budget
                </p>
              </div>
            </div>
          )}
