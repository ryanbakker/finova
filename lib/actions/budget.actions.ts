...
export async function createBudget(budgetData: CreateBudgetParams) {
  try {
    ...
    if (existingBudget) {
      throw new Error("A budget already exists for this category and time period");
    }
...
export async function updateBudget(budgetData: UpdateBudgetParams) {
  try {
    ...
    if (updateData.category || updateData.period || updateData.startDate || updateData.endDate) {
...
export async function getBudgetsByPeriod(period: "monthly" | "quarterly" | "yearly") {
  try {
    const { userId } = await auth();
