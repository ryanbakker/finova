...
export interface IAssetValueHistoryEntry {
  value: number;
  createdAt: Date;
}
...
export interface IAsset extends Document {
  ...
  valueHistory: IAssetValueHistoryEntry[];
  notes?: string;
  isActive: boolean;
  createdAt: Date;
  updatedAt: Date;
...
assetSchema.pre("save", function (next) {
  ...
  // Initialize valueHistory with initial value if empty
  if (!this.valueHistory || this.valueHistory.length === 0) {
    this.valueHistory = [
      {
        value: this.value,
        createdAt: new Date(),
  ...
  if (this.changePercentage === undefined || this.changePercentage === null) {
    this.changePercentage =
      this.value > 0 ? (this.changeAmount / this.value) * 100 : 0;
  }
...
assetSchema.pre(
  ...
  function (next) {
    ...
    if (update.accountNumber)
      update.accountNumber = update.accountNumber.trim();
...
assetSchema.virtual("totalReturnPercentage").get(function () {
  ...
  return this.currentValue
    ? ((this.currentValue - this.value) / this.value) * 100
    : 0;
});
